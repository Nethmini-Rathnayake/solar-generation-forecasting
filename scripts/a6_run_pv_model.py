...
from src.physics.pv_model import (
    ...
    build_noise_model, apply_synthetic_noise,
)
...
def _run_solcast_5min(cfg: dict, prefix: str) -> None:
    ...
    # ── Build NREL empirical noise model from overlap year ────────────────────
    logger.info("Building NREL empirical noise model from overlap period …")
    physics_overlap = pv_sim[pv_sim.index.isin(local_5min.index)]
    noise_model = build_noise_model(
        solcast_overlap, local_5min, physics_overlap,
    )

    # ── Inject site-matched noise into non-overlap synthetic rows ─────────────
    logger.info("Applying synthetic noise to non-overlap rows …")
    pv_sim = apply_synthetic_noise(
        pv_sim, solcast_cal, noise_model,
        actual_index=local_5min.index,
    )
    ...
    source_fig_dir     = resolve_path(cfg["paths"]["figures"]) / "a6_solcast"
    source_metrics_dir = resolve_path(cfg["paths"]["metrics"]) / "a6_solcast"
...
def main():
    ...
    source = args.source
    prefix = f"a6_{source}_" if source == "solcast" else f"{source}_"
